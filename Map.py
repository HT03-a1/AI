...
class HCMMapWindow(QMainWindow):
    ...
    # Lưu map ra file HTML
         self.map.save("hcm_map.html")

         self.map_widget = QWebEngineView(self)
    
    # Load file HTML vào widget
         html_path = QtCore.QDir.current().absoluteFilePath("hcm_map.html")
         self.map_widget.load(QtCore.QUrl.fromLocalFile(html_path))
    ...
    def draw(self, best_path):
        ...
        self.map.save("hcm_map.html")
        self.map_widget.load(QtCore.QUrl.fromLocalFile(QtCore.QDir.current().absoluteFilePath("hcm_map.html")))
